...
BUILD_CMD="$RC=$( ${BUILD_ROOT}/${PRODUCTION_SCRIPTS_DIR}/master-build.sh ${BUILD_ROOT}/${PRODUCTION_SCRIPTS_DIR}/build_eclipse_org.shsource 1>>$LOG_OUT_NAME 2>>$LOG_ERR_NAME )"
...
if [[ $RC != 0 ]]
then 
    echo "BUILD FAILED. RC: $RC.  See log files." >&2
fi
