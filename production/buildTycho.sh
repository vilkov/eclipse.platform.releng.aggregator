...
mvn clean install -Dmaven.test.skip=true -Dmaven.repo.local=$LOCAL_REPO -Dtycho.localArtifacts=default
cd ..
git clone http://git.eclipse.org/gitroot/tycho/org.eclipse.tycho.extras.git
cd org.eclipse.tycho.extras
mvn clean install -Dmaven.repo.local=$LOCAL_REPO -Dtycho.localArtifacts=default